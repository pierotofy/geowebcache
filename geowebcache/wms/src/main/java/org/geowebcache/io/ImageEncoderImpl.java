...
/**
 * Class implementing the ImageEncoder interface, the user should only create a
 * new bean for instantiating a new encoder object.
 */
public class ImageEncoderImpl implements ImageEncoder {
	...
	private static final Logger LOGGER = Logger
			.getLogger(ImageEncoderImpl.class);
	...
	/**
	 * Helper object used for preparing Image and ImageWriteParam for writing
	 * the image
	 */
	private WriteHelper helper;

	/**
	 * This enum is used for preparing the image to write (prepareImage()) and
	 * the related ImageWriteParam(prepareParams()).
	 */
	public enum WriteHelper {
		PNG("image/png", "image/png8", "image/png; mode=8bit", "image/png24",
			...
			public ImageWriteParam prepareParameters(ImageWriter writer,
					String compression, boolean compressUsed,
		...
		JPEG("image/jpeg") {
			protected ImageWriteParam prepareParameters(ImageWriter writer,
					String compression, boolean compressUsed,
					float compressionRate) {
		...
		public ImageWriteParam prepareParams(Map<String, String> inputParams,
				ImageWriter writer) {
			...
			boolean compressUsed = compression != null
					&& !compression.isEmpty()
			...
			ImageWriteParam params = prepareParameters(writer, compression,
					compressUsed, compressionRate);

			return params;
		}

		protected ImageWriteParam prepareParameters(ImageWriter writer,
				String compression, boolean compressUsed, float compressionRate) {
			// Parameters creation
			ImageWriteParam params = writer.getDefaultWriteParam();
			params.setCompressionMode(ImageWriteParam.MODE_EXPLICIT);
			if (compressUsed) {
	...
	/**
	 * Encodes the selected image with the defined output object. The user can
	 * set the aggressive outputStream if supported.
	 * 
	 * @param image
	 *            Image to write.
	 * @param destination
	 *            Destination object where the image is written.
	 * @param aggressiveOutputStreamOptimization
	 *            Parameter used if aggressive outputStream optimization must be
	 *            used.
	 * @throws IOException
	 */
	public void encode(RenderedImage image, Object destination,
			boolean aggressiveOutputStreamOptimization, MimeType type,
		...
		if (newSpi != null) {
			...
			try {
				...
				if (destination instanceof OutputStream) {
					...
					if (isAggressiveOutputStreamSupported()) {
						stream = new ImageOutputStreamAdapter(
								(OutputStream) destination);
					} else {
						stream = new MemoryCacheImageOutputStream(
								(OutputStream) destination);
					}
					...
					writer.write(null, new IIOImage(finalImage, null, null),
							params);
				} else {
	 ...
	 * 
	 * @return isAggressiveOutputStreamSupported Boolean indicating if the
	 *         selected encoder supports an aggressive output stream
	 *         optimization
	 */
	...
	/**
	 * Creates a new Instance of ImageEncoder supporting or not OutputStream
	 * optimization, with the defined MimeTypes and Spi classes.
	 * 
	 * @param aggressiveOutputStreamOptimization
	...
	public ImageEncoderImpl(boolean aggressiveOutputStreamOptimization,
		...
		// Checks for each Spi class if it is present and then it is added to
		// the list.
	...
	private static RenderedImage applyPalette(RenderedImage canvas) {
		if (!(canvas.getColorModel() instanceof IndexColorModel)) {
			...
			RenderedImage image = imageWorker.rescaleToBytes()
					.forceComponentColorModel().getRenderedImage();
			ColorIndexer indexer = new Quantizer(256).subsample()
					.buildColorIndexer(image);

			// if we have an indexer transform the image
			if (indexer != null) {
				image = new ImageWorker(image).colorIndex(indexer)
						.getRenderedImage();
			}
			return image;
		}
